...
/*************************************************
* u8bit Extraction Function                       *
*************************************************/
template<typename T> inline u8bit get_byte(u32bit byte_num, T input)
{ return (u8bit)(input >> ((sizeof(T)-1-(byte_num&(sizeof(T)-1))) << 3)); }

/*************************************************
* u8bit to Word Conversions                       *
*************************************************/
inline u16bit make_u16bit(u8bit input0, u8bit input1)
{ return (u16bit)(((u16bit)input0 << 8) | input1); }

inline u32bit make_u32bit(u8bit input0, u8bit input1, u8bit input2, u8bit input3)
{ return (u32bit)(((u32bit)input0 << 24) | ((u32bit)input1 << 16) |
				  ((u32bit)input2 <<  8) | input3); }

inline u64bit make_u64bit(u8bit input0, u8bit input1, u8bit input2, u8bit input3,
                          u8bit input4, u8bit input5, u8bit input6, u8bit input7)
{
...
template<typename T> inline void set_mem(T* ptr, u32bit n, u8bit val)
{ memset(ptr, val, sizeof(T)*n); }

template<typename T> inline s32bit cmp_mem(const T* p1, const T* p2, u32bit n)
{ return (memcmp(p1, p2, sizeof(T)*n)); }

void xor_buf(u8bit[], const u8bit[], u32bit);
void xor_buf(u8bit[], const u8bit[], const u8bit[], u32bit);

/*************************************************
* u8bit Swapping Functions                        *
*************************************************/
...
#define GETU32(pt) (((u32bit)(pt)[0] << 24) ^ ((u32bit)(pt)[1] << 16) ^ ((u32bit)(pt)[2] <<  8) ^ ((u32bit)(pt)[3]))
#define PUTU32(ct, st) { (ct)[0] = (u8bit)((st) >> 24); (ct)[1] = (u8bit)((st) >> 16); (ct)[2] = (u8bit)((st) >>  8); (ct)[3] = (u8bit)(st); }



void RIJNDAEL::InitializeCommon(void* pkey,int length)
{
//	Clear();
	const u32bit RC[10] = {
      0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000,
      0x40000000, 0x80000000, 0x1B000000, 0x36000000 };
	u8bit* key = static_cast<u8bit*>(pkey);
   Rounds = (length >> 2) + 6;

   u32bit XEK[64], XDK[64];

   const u32bit X = length >> 2;
   ...
   MoveMemory(EK,XEK+4,52 * sizeof(u32bit));
   MoveMemory(DK,XDK+4,52 * sizeof(u32bit));
}

void RIJNDAEL::EncryptCommon(void* inBlock,void* outBlock)
{
	u8bit* in = static_cast<u8bit*>(inBlock),
		*out = static_cast<u8bit*>(outBlock);
...
void RIJNDAEL::DecryptCommon(void* inBlock,void* outBlock)
{
	u8bit* in = static_cast<u8bit*>(inBlock),
		*out = static_cast<u8bit*>(outBlock);
	 u32bit T0, T1, T2, T3, B0, B1, B2, B3;
